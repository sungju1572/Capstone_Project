...
dir_path = "C:\\Users\\sjyoo\\Desktop\\training_dataset"
file_list = []
...
file_list_split = list(map(lambda x : x.split('\\')[5].split('.')[0], file_list))
...
for file in file_list:
    ...
    #가우시안 블러 사용 - 노이즈 줄이기
    img_blurred = cv2.GaussianBlur(gray, ksize=(5, 5), sigmaX=0) #0,1,2,3,4...
    ...
    MAX_ANGLE_DIFF = 15.0    #두 사각형의 중심을 그었을때 쎄타(각도) 의 최댓값
    MAX_AREA_DIFF = 0.5      #면적차이 (0.5보다 크면 인정x)
    MAX_WIDTH_DIFF = 0.8     #너비차이 (..)
    MAX_HEIGHT_DIFF = 0.2    #높이차이
    MIN_N_MATCHED = 4       #위에 조건을 만족하는 애들이 최소 3개 이상
